...
def translate_image_to_single_band(image):
    ...
    for i in range(image.size[0]):
        for j in range(image.size[1]):
            ...
            temp_array2 = np.asarray((30, 59, 11))
...
def translate_p_mode_to_single(image):
    ...
    for i in range(image.size[1]):
        for j in range(image.size[0]):
            for k in range(3):
                if image_array[i, j,k] > 200:
                    image_array[i, j,k] = 0
                else:
                    image_array[i, j,k]=255-image_array[i, j,k]
...
def draw_line(slide_window_array, index, raw_image):
    ...
    for i in range(raw_image.size[1]):
        raw_image_pix[int(split_line_x), i] = 177
...
def main():
    answer_file_handler = open('D:/project/VertCode/1(1).txt')
    answer_file_list = answer_file_handler.readlines()
    answer_file_handler.close()

    for i in range(2000):
        print i
        image = Image.open('D:/project/VertCode/tuniu' + '/'+str(i+1000) + '.jpg')
        ...
        split_image_list = split_character_by_mean(slide_window_array, image, 4)
        answer = answer_file_list[i].split('\t')[1][:4]

        for j in range(4):
            split_image_list[j].resize((24,28)).save('D:/project/VertCode/checkcode' + '/'+ answer[j]  + str(i) + str(j) + '.gif')
            if split_image_list[j].resize((24,28)).size != (24,28):
                print split_image_list[j].resize((24,28)).size,j
